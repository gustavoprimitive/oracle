  ...
  */
  ...
  --Procedimineto local para output de log
  ...
  BEGIN
    ...
    IF v_count = 0 OR v_num_rec IS NULL THEN
      --Otros eventos
      dbms_output.put_line('[' || to_char(SYSDATE, 'DD/MM/YYYY HH24:MI:SS') || '] ' || $$PLSQL_UNIT || chr(9) || v_comments);
...
BEGIN
  ...
  dbms_output.put_line('[' || to_char(SYSDATE, 'DD/MM/YYYY HH24:MI:SS') || '] ' || $$PLSQL_UNIT || chr(9) || 'Fin de carga a tabla destino');
  dbms_output.put_line('[' || to_char(SYSDATE, 'DD/MM/YYYY HH24:MI:SS') || '] ' || $$PLSQL_UNIT || chr(9) || 'Total: ' || v_sum || ' registro(s) en ' || v_count || ' bloque(s)');
